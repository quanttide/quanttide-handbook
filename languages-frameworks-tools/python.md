...
源素材建议使用`poetry`作为包管理工具。它用于管理项目依赖、构建和发布。
...
类命名中，源素材特别讨论了 API、SDK、OSRM、COS、EIAM 等缩写：
...
缓存框架源素材里对比了标准库`tempfile`和社区库`diskcache`。
...
源素材建议使用`pytest`或`unittest`进行单元测试。
