...
量潮工作手册研发目录下的 DevOps 子手册，讲的是怎么把版本、代码、构建、测试、发布、部署、运维和监控连成一条可执行的线。

## 适用范围

- 适用于使用 `qtcloud-devops`、GitHub Actions 或类似 CI/CD 流程管理项目的场景。
- 适用于需要把版本号、配置文件、CHANGELOG、tag 和制品统一起来的项目。
- 适用于多个 scope 共存、需要分别发布和分别审计的仓库。

## 工作原则

- 版本、配置、CHANGELOG 和 tag 要互相对得上。
- 自动化优先，人工只做例外处理。
- 每个关键动作都要留痕。
- 发布前先审计，发布后再验收。
